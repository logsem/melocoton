...
Definition can_link T1 T2 T3 Tres (p1 p2 p3 : gmap string (Λ.(func))) :iProp Σ := 
   ⌜dom p1 ## dom p2⌝
 ∗ □(∀ s vv Φ, T3 s vv Φ -∗ ⌜s ∉ dom p1⌝ ∗ ⌜s ∉ dom p2⌝)
 ∗ □(∀ s vv Φ, Tres s vv Φ -∗ (spec_union T1 T2) s vv Φ)
 ∗ □(spec_union T2 T3 |- p1 :: T1)
 ∗ □(spec_union T1 T3 |- p2 :: T2)
 ∗ ⌜p3 = (union_with (λ _ _ : func Λ, None) p1 p2)⌝.

Lemma wp_link_execs T1 T2 T3 Tres (p1 p2 p3 : gmap string (Λ.(func))) :
  can_link T1 T2 T3 Tres p1 p2 p3
 -∗ ∀ e Φ,
    WP e @ mkPe p1 (spec_union T2 T3); ⊤ {{ v, Φ v }} ∨ WP e @ mkPe p2 (spec_union T1 T3); ⊤ {{ v, Φ v }} -∗
    WP e @ mkPe p3 (T3); ⊤ {{ v, Φ v }}.
Proof.
  iIntros "(%Hdis & #HT3 & _ & #Hp1 & #Hp2 & ->)".
...
Lemma wp_link_progs T1 T2 T3 Tres (p1 p2 p3 : gmap string (Λ.(func))) :
    can_link T1 T2 T3 Tres p1 p2 p3
 -∗ T3 |- p3 :: Tres.
Proof.
  iIntros "(%Hdis & #HT3 & #Hres & #Hp1 & #Hp2 & ->)".
  iIntros (s vv Φ) "H". iDestruct ("Hres" with "H") as "[H|H]".
  - iPoseProof ("Hp1" with "H") as "(%HNone & HH)".
    ...
    * iSplitR.
      ...
      -- iApply (wp_link_execs with "[HT3 Hp2] [HH]"); last by iLeft. unfold can_link; repeat iSplit; (try by iPureIntro); done.
  - iDestruct ("Hp2" with "H") as "(%HNone & HH)".
    ...
    * iSplitR.
      ...
      -- iApply (wp_link_execs with "[HT3 Hp2] [HH]"); last by iRight. unfold can_link; repeat iSplit; (try by iPureIntro); done.
Qed.
