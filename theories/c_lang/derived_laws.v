...
Lemma wp_Malloc s E n :
  ...
  {{{ l, RET OVal (LitV (LitLoc l)); l I↦C∗ replicate (Z.to_nat n) None }}}.
Proof.
...
Lemma wp_Malloc_vec s E n :
  ...
  {{{ True }}}
    Malloc #n @ s ; E
  {{{ l, RET OVal #l; l I↦C∗ vreplicate (Z.to_nat n) None }}}.
Proof.
...
Lemma wp_load_offset s E l dq off vs (v:val) :
  vs !! off = Some (Some v) →
  {{{ ▷ l I↦C∗{dq} vs }}} * #(l +ₗ off) @ s; E {{{ RET OVal v; l I↦C∗{dq} vs }}}%CE.
Proof.
...
Lemma wp_store_offset s E l off vs (v:val) :
  off < length vs →
  {{{ ▷ l I↦C∗ vs }}} #(l +ₗ off) <- v @ s; E {{{ RET OVal #0; l I↦C∗ <[off:=Some v]> vs }}}%CE.
Proof.
...
Lemma wp_store_offset_vec s E l sz (off : fin sz) (vs : vec (option val) sz) (v:val) :
  {{{ ▷ l I↦C∗ vs }}} #(l +ₗ off) <- v @ s; E {{{ RET OVal #0; l I↦C∗ vinsert off (Some v) vs }}}%CE.
Proof.
...
Lemma wp_free_array s E l vs :
  {{{ ▷ l I↦C∗ vs }}} free (#l, #(Z.of_nat (length vs))) @ s; E {{{ RET OVal (LitV LitUnit); True }}}%CE.
Proof.
  iIntros (Φ) ">Hl HΦ".
...
Lemma wp_free_array' s E l z vs :
  ...
  {{{ ▷ l I↦C∗ vs }}} free (#l, #z) @ s; E {{{ RET OVal (LitV LitUnit); True }}}%CE.
