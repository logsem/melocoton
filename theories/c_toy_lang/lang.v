...
Inductive head_step (p : program) : expr → state → expr → state → Prop :=
  ...
  | FunCallS s va args res e σ :
     ...
     zip_args args va = Some res →
     head_step p (FunCall (Val $ LitV $ LitFunPtr s) (map Val va)) σ (subst_all res e) σ.
...
(** Language *)
Canonical Structure C_ectxi_lang p := EctxiLanguage (C_lang.C_lang_mixin p).
