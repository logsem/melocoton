...
From melocoton.c_toy_lang Require Import iris.proofmode notation.
From iris.prelude Require Import options.

Class heapGpreS Σ := HeapGpreS {
  heapGpreS_iris :> invGpreS Σ;
  heapGpreS_heap :> gen_heapGpreS loc heap_cell Σ;
  heapGpreS_inv_heap :> inv_heapGpreS loc heap_cell Σ;
  heapGpreS_proph :> proph_mapGpreS positive (prod val val) Σ;
  heapGS_step_cnt :> mono_natG Σ;
}.

Definition heapΣ : gFunctors :=
  #[invΣ; gen_heapΣ loc heap_cell; inv_heapΣ loc heap_cell;
    proph_mapΣ positive (prod val val); mono_natΣ].
Global Instance subG_heapGpreS {Σ} {p:program} : subG heapΣ Σ → heapGpreS Σ.
Proof. solve_inG. Qed.
...
Definition heap_adequacy Σ `{!heapGpreS Σ} {p:program} s e σ φ :
