...
Section FFI_spec.
  ...
  Definition caml_incr : expr :=
    "p" <- *"p" + #C 1;;
    #C LitNull.

  (* Times two plus one *)
  Definition caml_ttpo : expr :=
    let: "e2" := "e" in
    call: "caml_incr" with (&: "e");;
    "e2" + "e".

  Definition caml_local_prog : lang_prog C_lang :=
    {[
      "caml_incr" := Fun [BNamed "p"] caml_incr;
      "caml_ttpo" := Fun [BNamed "e"] caml_ttpo
    ]}.

  Definition caml_local_spec : protocol ML_lang.val Σ :=
    !! e (n : Z)
      {{ ⌜e = #ML n⌝ }}
        "caml_ttpo" with [e]
      {{
  ...
  Lemma caml_local_correct:
    prims_proto caml_local_spec
    ||- caml_local_prog :: wrap_proto caml_local_spec.
  Proof.
