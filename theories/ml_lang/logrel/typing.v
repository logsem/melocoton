...
Lemma typed_ind (P : program_env → gmap string type → expr → type → Prop) : 
  ...
  → (∀ (P0 : program_env) (Γ : gmap string type) (x : binder) (e1 e2 : expr) (τ : {bind type}) (τ' : type), P0; Γ ⊢ₜ e1 : TExist τ → P P0 Γ e1 (TExist τ) → subst_prog_env (ren (+1)) P0; binder_insert x τ (subst (ren (+1)) <$> Γ) ⊢ₜ e2 : τ'.[ren (+1)] → P (subst_prog_env (ren (+1)) P0) (binder_insert x τ (subst (ren (+1)) <$> Γ)) e2 τ'.[ren (+1)] → P P0 Γ (unpack: x := e2 in e1)%MLE τ')
  ...
  → (∀ (P0 : program_env) (Γ : gmap string type) (e : expr) (τ : type), P0; Γ ⊢ₜ e : τ → P P0 Γ e τ → P P0 Γ (ref e)%MLE (Tref τ))
  → (∀ (P0 : program_env) (Γ : gmap string type) (e : expr) (τ : type), P0; Γ ⊢ₜ e : Tref τ → P P0 Γ e (Tref τ) → P P0 Γ ((! e)%MLE #0) τ)
  → (∀ (P0 : program_env) (Γ : gmap string type) (e e' : expr) (τ : type), P0; Γ ⊢ₜ e : Tref τ → P P0 Γ e (Tref τ) → P0; Γ ⊢ₜ e' : τ → P P0 Γ e' τ → P P0 Γ ((e <- #0)%MLE e') TUnit)
