...
Lemma wp_lift_pure_det_step_no_fork `{!Inhabited (state Λ)} {s E Φ} e1 e2 :
  ...
  (∀ σ1 e2' σ2, matching_expr_state e1 σ1 →
    prim_step (prog s) e1 σ1 e2' σ2 →
    σ2 = σ1 ∧ e2' = e2) →
...
Proof.
  ...
  iModIntro.
  destruct (Hstep σ1 e' σ2 Hmatch Hstep') as (He1 & He2). subst.
  iFrame.
Qed.

  Record pure_step (p : mlanguage.prog Λ) (e1 e2 : expr Λ) := {
    pure_step_safe σ1 : reducible p e1 σ1;
    pure_step_det σ1 e2' σ2 :
      prim_step p e1 σ1 e2' σ2 → σ2 = σ1 ∧ e2' = e2
  }.

  Record pure_head_step (p : mlanguage.prog Λ) (e1 e2 : expr Λ) := {
...
Class PureExec (φ : Prop) (n : nat)  (p : mlanguage.prog Λ) (e1 e2 : expr Λ) :=
