...
(*
  Definition reducible_old (p : prog Λ) (e : expr Λ) (σ : state Λ) :=
    ∃ X, prim_step p (e, σ) X. *)
 ...
 (* Definition not_stuck_old (p : prog Λ) (e : expr Λ) (σ : state Λ) :=
    is_Some (to_val e) ∨ reducible_old p e σ. *)
...
(*
  Lemma not_reducible_old p e σ : ¬reducible_old p e σ ↔ irreducible p e σ.
  Proof. unfold reducible_old, irreducible. naive_solver. Qed.
  Lemma reducible_old_not_val p e σ : reducible_old p e σ → to_val e = None.
  ...
  (*
  Lemma not_not_stuck p e σ : ¬not_stuck_old p e σ ↔ stuck p e σ.
  Proof.
    rewrite /stuck /not_stuck_old -not_eq_None_Some -not_reducible_old.
...
(*
  ...
  Lemma head_step_not_stuck p e σ X:
    head_step p (e, σ) X → not_stuck_old p e σ.
  Proof. rewrite /not_stuck_old /reducible_old /=. eauto 10 using head_prim_step. Qed. *)
  ...
  Qed.
(*
  Lemma fill_reducible_old p K e σ : reducible_old p e σ → reducible_old p (fill K e) σ.
  Proof.
    intros (X & Hstep). exists (λ '(e', σ), ∃ e, e' = fill K e ∧ X (e, σ)).
    eapply fill_prim_step; [| apply Hstep]; eauto.
  Qed.
  Lemma head_prim_reducible_old p e σ : head_reducible p e σ → reducible_old p e σ.
  Proof. intros (X&?). exists X. by apply head_prim_step. Qed.
  Lemma head_prim_fill_reducible_old p e K σ :
    head_reducible p e σ → reducible_old p (fill K e) σ.
  Proof. intro. by apply fill_reducible_old, head_prim_reducible_old. Qed.
  Lemma head_prim_irreducible p e σ : irreducible p e σ → head_irreducible p e σ.
  Proof.
    rewrite -not_reducible_old -not_head_reducible. eauto using head_prim_reducible_old.
  Qed. *)
  ...
  Lemma prim_head_reducible p e σ :
    reducible_old p e σ → sub_redexes_are_values e → head_reducible p e σ.
  ...
  Proof.
    rewrite -not_reducible_old -not_head_reducible. eauto using prim_head_reducible.
  Qed. 
  ...
  Lemma head_reducible_prim_step_ctx p K e1 σ1 X X' :
    head_step p (e1, σ1) X' →
    prim_step p (fill K e1, σ1) X →
  ...
  Lemma head_reducible_prim_step p e1 σ1 X X' :
    head_step p (e1, σ1) X' →
    prim_step p (e1, σ1) X →
    head_step p (e1, σ1) X.
  Proof.
    intros Hred Hpstep.
    epose proof (head_reducible_prim_step_ctx _ empty_ectx _ _ X X' Hred) as Hhstep.
  ...
  Lemma fill_not_stuck (P : prog Λ) e σ K : not_stuck_old P (fill K e) σ → not_stuck_old P e σ.
  Proof.
    intros [Hval | Hred].
  ...
  Lemma fill_reducible_old_prim_step p e σ K X :
    reducible_old p e σ →
  ...
  Lemma reducible_fill p e σ K : reducible_old p e σ → reducible_old p (fill K e) σ.
...
(*
  Lemma fill_reducible_inv p K e σ :
    to_val e = None -> reducible_old p (fill K e) σ → reducible_old p e σ.
