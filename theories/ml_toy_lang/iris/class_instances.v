...
Proof. done. Qed.
Global Instance as_val_val {p:ml_program} v : AsVal (Val v).
...
Section pure_exec.
  ...
  Global Instance pure_funcall s va f res : 
    PureExec ((p : gmap.gmap string ml_function) !! s = Some f ∧ apply_function f va = Some res) 1 (Extern s (map Val va)) res.
  Proof. destruct f. subst; intros ?; apply nsteps_once, pure_head_step_pure_step;
      constructor;  [intros; subst; do 3 eexists; econstructor; eauto | simpl; intros; inv_head_step].
    + destruct H as [H1 H2]. apply H1.
    + destruct H as [H1 H2]; apply H2.
    + repeat split. destruct H.
      assert (args=b) as -> by congruence.
      destruct (zip_args b va); last congruence.
      congruence.
  Qed.
End pure_exec.
