...
From melocoton.ml_toy_lang Require Export melocoton.primitive_laws.
...
Proof.
  iIntros (Hzs Φ) "_ HΦ". iApply wp_allocN_seq; [done..|]. iModIntro.
...
Lemma wp_allocN_vec s E v n :
  ...
  {{{ l, RET #l; l ↦∗ vreplicate (Z.to_nat n) v ∗
         [∗ list] i ∈ seq 0 (Z.to_nat n), meta_token (l +ₗ (i : nat)) ⊤ }}}.
Proof.
  iIntros (Hzs Φ) "_ HΦ". iApply wp_allocN; [ lia | done | .. ].
...
Proof.
  ...
  iApply (wp_load_offset' with "H"); [by eauto..|]; iIntros "!> H HΦ".
  by iApply "HΦ".
Qed.
...
Proof.
  iIntros (? Φ) ">H HΦ". iApply (wp_step with "HΦ").
  iApply (wp_store_offset' with "H"); [by eauto..|]; iIntros "!> H HΦ".
  by iApply "HΦ".
Qed.
...
Proof.
  iIntros (Φ) ">H HΦ". iApply (wp_step with "HΦ").
  iApply (wp_store_offset_vec' with "H"); [by eauto..|]; iIntros "!> H HΦ".
  by iApply "HΦ".
Qed.
(*
