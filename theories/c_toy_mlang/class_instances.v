...
Section pure_exec.
  ...
  Global Instance pure_let x (v1 v2 : val) :
    PureExec True 1 p (Let x (Val v1) (Val v2)) (only (subst' x v1 v2)).
  Proof. solve_pure_exec. Qed.

  Global Instance pure_unop op v v' :
    PureExec (un_op_eval op v = Some v') 1 p (UnOp op (Val v)) (only (Val v')).
  Proof. solve_pure_exec. Qed.

  Global Instance pure_binop op v1 v2 v' :
    PureExec (bin_op_eval op v1 v2 = Some v') 1 p (BinOp op (Val v1) (Val v2)) (only (Val (LitV v'))) | 10.
  Proof. solve_pure_exec. Qed.
  ...
  Global Instance pure_eqop v1 v2 :
    PureExec (vals_compare_safe v1 v2) 1 p
      ...
      (only (Val $ LitV $ LitBool $ bool_decide (v1 = v2))) | 1.
  Proof.
    intros Hcompare.
