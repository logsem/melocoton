...
Program Instance link_mlangGS : mlangGS hlc val Σ (link_lang Λ1 Λ2) := {
  state_interp := link_state_interp;
  at_boundary := link_in_state Boundary;
}.

Program Instance link_linkableGS : linkableGS (link_lang Λ1 Λ2) public_state_interp := {
...
Definition link_environ
  (pe1 : prog_environ Λ1)
  (pe2 : prog_environ Λ2)
: prog_environ (link_lang Λ1 Λ2) :=
{|
...
:
  ...
  WP Link.LkE (Link.RunFunction Λ1 Λ2 fn vs) k @ pe; E {{ Φ }} -∗
  WP Link.LkE (Link.ExprCall Λ1 Λ2 fname vs) k @ pe; E {{ Φ }}.
Proof using.
  iIntros (pe Hfn) "Hwp".
...
:
  ...
  (∀ e1, ⌜apply_func fn arg = Some e1⌝ -∗
         ...
         link_state_frag In1 -∗
         WP Link.LkE (Link.Expr1 Λ1 Λ2 e1) (inr k2 :: k) @ pe; E {{ Φ }}) -∗
  WP Link.LkE (Link.RunFunction Λ1 Λ2 (inl fn) arg) (inr k2 :: k) @ pe; E {{ Φ }}.
Proof using.
...
:
  ...
  (∀ e2, ⌜apply_func fn arg = Some e2⌝ -∗
         ...
         link_state_frag In2 -∗
         WP Link.LkE (Link.Expr2 Λ1 Λ2 e2) (inl k1 :: k) @ pe; E {{ Φ }}) -∗
  WP Link.LkE (Link.RunFunction Λ1 Λ2 (inr fn) arg) (inl k1 :: k) @ pe; E {{ Φ }}.
Proof using.
...
Lemma wp_link_retval_1
  (pe1 : prog_environ Λ1)
  (pe2 : prog_environ Λ2)
  E k1 v (Φ : val → iProp Σ)
:
  let pe := link_environ pe1 pe2 in
  (link_state_frag In1 -∗ WP LkSE (Link.Expr1 Λ1 Λ2 (fill k1 (of_val Λ1 v))) @ pe; E {{ Φ }}) -∗
  (link_state_frag Boundary -∗ WP Link.LkE (Link.ExprV Λ1 Λ2 v) [inl k1] @ pe; E {{ Φ }}).
Proof using.
  iIntros (pe) "Hwp Hb".
  iApply wp_unfold. rewrite /wp_pre.
...
Lemma wp_link_retval_2
  (pe1 : prog_environ Λ1)
  (pe2 : prog_environ Λ2)
  E k2 v (Φ : val → iProp Σ)
:
  let pe := link_environ pe1 pe2 in
  (link_state_frag In2 -∗ WP LkSE (Link.Expr2 Λ1 Λ2 (fill k2 (of_val Λ2 v))) @ pe; E {{ Φ }}) -∗
  (link_state_frag Boundary -∗ WP Link.LkE (Link.ExprV Λ1 Λ2 v) [inr k2] @ pe; E {{ Φ }}).
Proof using.
  iIntros (pe) "Hwp Hb".
  iApply wp_unfold. rewrite /wp_pre.
  iIntros (σ) "Hσ". iDestruct (link_at_boundary with "[$Hσ $Hb]") as %(pubσ&privσ1&privσ2&->).
...
:
  ...
  (▷ ∀ r, WP fill k1 (of_class Λ1 (ExprVal r)) @ pe1; E {{ λ v, Φ v ∗ at_boundary Λ1 }} -∗
        link_state_frag Boundary -∗
        at_boundary Λ2 -∗
        WP Link.LkE (Link.ExprV Λ1 Λ2 r) [inl k1] @ pe; E {{ λ v, Φ v ∗ at_boundary (link_lang Λ1 Λ2) }}) -∗
  at_boundary (link_lang Λ1 Λ2) -∗
  WP Link.LkE (Link.ExprCall Λ1 Λ2 fn_name arg) [inl k1] @ pe; E {{ λ v, Φ v ∗ at_boundary (link_lang Λ1 Λ2) }}.
Proof using.
...
:
  ...
  (▷ ∀ r, WP fill k2 (of_class Λ2 (ExprVal r)) @ pe2; E {{ λ v, Φ v ∗ at_boundary Λ2 }} -∗
        link_state_frag Boundary -∗
        at_boundary Λ1 -∗
        WP Link.LkE (Link.ExprV Λ1 Λ2 r) [inr k2] @ pe; E {{ λ v, Φ v ∗ at_boundary (link_lang Λ1 Λ2) }}) -∗
  at_boundary (link_lang Λ1 Λ2) -∗
  WP Link.LkE (Link.ExprCall Λ1 Λ2 fn_name arg) [inr k2] @ pe; E {{ λ v, Φ v ∗ at_boundary (link_lang Λ1 Λ2) }}.
Proof using.
  iIntros (pe Hfn1 Hfn2) "HTΞ HΞ Hwp (Hb & Hb1 & Hb2)".
  assert (Hpef: prog pe !! fn_name = None).
  { apply lookup_union_None. rewrite !lookup_fmap Hfn1 Hfn2 //. }
...
Lemma link_head_step_call_ext_1_inv pe1 pe2 K f vs σ1 privσ2 X :
  let pe := link_environ pe1 pe2 in
  can_link_prog pe1 pe2 →
  prog pe1 !! f = None →
  head_step (prog pe) (LkSE (Link.Expr1 Λ1 Λ2 (fill K (of_class Λ1 (ExprCall f vs)))),
                       Link.St1 σ1 privσ2) X →
  ∃ pubσ privσ1 k,
    K = comp_ectx k empty_ectx ∧
    X (Link.LkE (Link.ExprCall Λ1 Λ2 f vs) [inl k], Link.St pubσ privσ1 privσ2) ∧
...
Lemma link_head_step_call_ext_2_inv pe1 pe2 K f vs σ2 privσ1 X :
  let pe := link_environ pe1 pe2 in
  can_link_prog pe1 pe2 →
  prog pe2 !! f = None →
  head_step (prog pe) (LkSE (Link.Expr2 Λ1 Λ2 (fill K (of_class Λ2 (ExprCall f vs)))),
                       Link.St2 privσ1 σ2) X →
  ∃ pubσ privσ2 k,
    K = comp_ectx k empty_ectx ∧
    X (Link.LkE (Link.ExprCall Λ1 Λ2 f vs) [inr k], Link.St pubσ privσ1 privσ2) ∧
...
:
  ...
  ⊢ □ (
    (∀ e1 Φ, WP e1 @ pe1; E {{ λ v, Φ v ∗ at_boundary Λ1 }} -∗ link_in_state In1 -∗
             WP (LkSE (Link.Expr1 Λ1 Λ2 e1)) @ pe; E {{ λ v, Φ v ∗ link_in_state Boundary }}) ∗
    (∀ e2 Φ, WP e2 @ pe2; E {{ λ v, Φ v ∗ at_boundary Λ2 }} -∗ link_in_state In2 -∗
             WP (LkSE (Link.Expr2 Λ1 Λ2 e2)) @ pe; E {{ λ v, Φ v ∗ link_in_state Boundary }})
  ).
Proof using.
  ...
  { iIntros (e1 Φ) "Hwp [Hb Hb2]".
    ...
    { iDestruct "Hwp" as (v ->) "[Hσ [HΦ Hb1]]".
      ...
      assert (Hhred: head_reducible (prog pe) (LkSE (Link.Expr1 Λ1 Λ2 (of_class Λ1 (ExprVal v)))) (Link.St1 σ1 privσ2)).
    ...
    { iDestruct "Hwp" as (f vs K -> Hf) "(Hb1 & Hσ1 & Hcall)". iModIntro. iRight; iRight.
      ...
      assert (head_reducible (prog pe) (LkSE (Link.Expr1 Λ1 Λ2 (fill K (of_class Λ1 (ExprCall f vs))))) (Link.St1 σ1 privσ2)).
      ...
      { (* call to a function of pe2 *)
        ...
        progress change (Link.LkE (Link.Expr2 Λ1 Λ2 e2) [inl k1]) with
          (Link.fill _ _ ([inl k1]:ectx (link_lang Λ1 Λ2)) (LkSE (Link.Expr2 Λ1 Λ2 e2))).
        iApply wp_bind.
    ...
    { (* WP: step case *)
      ...
      assert (head_reducible (prog pe) (LkSE (Link.Expr1 Λ1 Λ2 e1)) (Link.St1 σ1 privσ2)).
      { destruct Hred as (? & Hstep). exists (λ _, True).
        eapply Link.Step1S. rewrite proj1_prog_union; [| apply Hcan_link]. all: eauto. }
      ...
      { exfalso. clear Hstep. erewrite <-(of_to_class e0) in Hred; [|eassumption].
        rewrite proj1_prog_union in H4;[|apply Hcan_link].
  ...
  { iIntros (e2 Φ) "Hwp [Hb Hb1]".
    ...
    { iDestruct "Hwp" as (v ->) "[Hσ [HΦ Hb2]]". iModIntro. iRight; iRight.
      ...
      assert (Hhred: head_reducible (prog pe) (LkSE (Link.Expr2 Λ1 Λ2 (of_class Λ2 (ExprVal v)))) (Link.St2 privσ1 σ2)).
    ...
    { iDestruct "Hwp" as (f vs K -> Hf) "(Hb2 & Hσ2 & Hcall)". iModIntro. iRight; iRight.
      ...
      assert (head_reducible (prog pe) (LkSE (Link.Expr2 Λ1 Λ2 (fill K (of_class Λ2 (ExprCall f vs))))) (Link.St2 privσ1 σ2)).
      ...
      { (* call to a function of pe1 *)
        ...
        progress change (Link.LkE (Link.Expr1 Λ1 Λ2 e1) [inr k2]) with
          (Link.fill _ _ ([inr k2]:ectx (link_lang Λ1 Λ2)) (LkSE (Link.Expr1 Λ1 Λ2 e1))).
        iApply wp_bind.
    ...
    { (* WP: step case *)
      iDestruct "Hwp" as (Hred) "Hwp". iModIntro. iRight; iRight.
      assert (head_reducible (prog pe) (LkSE (Link.Expr2 Λ1 Λ2 e2)) (Link.St2 privσ1 σ2)).
      { destruct Hred as (? & Hstep).
        exists (λ _, True). eapply Link.Step2S. rewrite proj2_prog_union; [| apply Hcan_link].
        all: eauto. }
      iSplitR; [by iPureIntro; apply head_prim_reducible|].
      iIntros (X Hstep). apply head_reducible_prim_step in Hstep;[|done].
      inversion Hstep; simplify_eq.
      ...
      { exfalso. apply reducible_not_val in Hred. congruence. }
      { exfalso. clear Hstep. erewrite <-(of_to_class e0) in Hred; [|eassumption].
        rewrite proj2_prog_union in H4;[|apply Hcan_link].
        destruct Hred as (?&Hred). eapply extcall_ctx_irreducible; eauto. } } }
Qed.
