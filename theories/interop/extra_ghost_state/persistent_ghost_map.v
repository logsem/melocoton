...
Class pgmG `{SI: indexT} Σ (K V Vpers : Type) (Fpers : V → Vpers) `{Countable K} := GhostMapG {
  pgm_inG : inG Σ (gmap_view_plusR K (leibnizO V) (leibnizO Vpers) Fpers);
}.
Local Existing Instance pgm_inG.

Definition pgmΣ `{SI: indexT} (K V Vpers : Type) (Fpers : V → Vpers) `{Countable K} : gFunctors :=
  #[ GFunctor (gmap_view_plusR K (leibnizO V) (leibnizO Vpers) Fpers) ].

Global Instance subG_pgmΣ `{SI: indexT} Σ (K V Vpers : Type) (Fpers : V → Vpers) `{Countable K} :
  subG (pgmΣ K V Vpers Fpers) Σ → pgmG Σ K V Vpers Fpers.
Proof. solve_inG. Qed.

Section definitions.
  Context `{pgmG Σ K V Vpers Fpers}.

  Local Definition pgm_auth_def
      (γ : gname) (q : Qp) (m : gmap K V) : iProp Σ :=
    own γ (gmap_view_plus_auth (V:=leibnizO V) (Vpers := leibnizO Vpers) (Fpers := Fpers) (DfracOwn q) m).
  ...
  Local Definition pgm_elem_def
    ...
    own γ (gmap_view_plus_frag (V:=leibnizO V) (Vpers := leibnizO Vpers) (Fpers := Fpers) k dq v).
  ...
  Local Definition pgm_pers_def
    ...
    own γ (gmap_view_plus_pers (V:=leibnizO V) (Vpers := leibnizO Vpers) (Fpers := Fpers) k vp).
...
Section lemmas.
  ...
  Local Lemma pgm_elems_unseal γ m dq :
    ([∗ map] k ↦ v ∈ m, k □↪[γ]{dq} v) ==∗
    own γ ([^op map] k↦v ∈ m, gmap_view_plus_frag (V:=leibnizO V) k dq v).
  Proof.
  ...
  Local Lemma pgm_perms_unseal γ (mpers : gmap K Vpers) dq :
    ...
    own γ ([^op map] k↦vp ∈ mpers, gmap_view_plus_pers (V:=leibnizO V) (Vpers:=leibnizO Vpers) k vp).
  Proof.
  ...
  Proof.
    unseal. iIntros "Helem1 Helem2".
    iDestruct (own_valid_2 with "Helem1 Helem2") as %?%gmap_view_plus_pers_op_valid_L.
    iPureIntro; apply H1.
  Qed.
  ...
  Proof.
    unseal. intros.
    iMod (own_alloc_strong (gmap_view_plus_auth (V:=leibnizO V) (DfracOwn 1) ∅) P)
      as (γ) "[% Hauth]"; first done.
    { apply gmap_view_plus_auth_valid. }
    iExists γ. iSplitR; first done.
    rewrite -big_opM_own_1 -own_op. iApply (own_update with "Hauth").
    etrans; first apply: (gmap_view_plus_alloc_big (V:=leibnizO V) _ m (DfracOwn 1)).
  ...
  Proof.
    ...
    eapply gmap_view_plus_pers_both_dfrac_validN in HH as (Hq&v&Hv1&Hv2).
  ...
  Proof.
    iIntros (?) "Hauth".
    iMod (pgm_insert k with "Hauth") as "[$ Helem]"; first done.
  ...
  Proof.
    unseal. intros Hpers. apply bi.wand_intro_r. rewrite -!own_op.
    apply own_update. apply: gmap_view_plus_update. done.
  Qed.
...
(*
  ...
  Proof.
    ...
    apply: gmap_view_plus_delete_big.
  Qed. *)
